...
class FirestoreHelper {
  ...
  Future<List<Location>> selectAllLocations(String runId) async {
    final db = FirebaseFirestore.instance;
    final snapshot = db.collection("runs").doc(runId).collection("locations").withConverter(
  ...
  Future<Location?> getLocationData(String runId, String locationId) async {
    final db = FirebaseFirestore.instance;
    final docRef = db.collection("runs").doc(runId).collection("locations").doc(locationId).withConverter(
      fromFirestore: Location.fromFirestore,
      toFirestore: (Location location, _) => location.toFirestore(),
    );
  ...
  Future<void> insertLocation(String runId, Location location) async {
    final db = FirebaseFirestore.instance;
    final docRef = db.collection("runs").doc(runId).collection("locations").doc(location.id).withConverter(
      fromFirestore: Location.fromFirestore,
  ...
  Future<void> deleteLocation(String runId, String locationId) async {
    final db = FirebaseFirestore.instance;
    await db.collection("runs").doc(runId).collection("locations").doc(locationId).delete();
  }

  // 応援メッセージを取得する
  Future<List<Message>> selectAllMessages(String runId) async {
    final db = FirebaseFirestore.instance;
    final snapshot = db.collection("messages").where('runId', isEqualTo: runId).withConverter(
